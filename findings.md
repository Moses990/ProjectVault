...
- 待集成差异中的新增 `docs/reviews/` 包含本机绝对路径、真实项目名称、真实文件名、清单及截图。
- 仓库为公开仓库；即使后续提交删除，先推送原始阶段历史仍会使敏感 blob 可从 Git 历史访问。
- 因此不能直接推送原始分支或阶段标签。集成必须从 `origin/main` 生成新的 squash 分支，并在提交前完全排除新增 `docs/reviews/`。
- 原始分支和标签留在本机，保留验收追溯；公开分支只含产品代码、测试、通用规范和脱敏连续性文档。
- 已执行通用密钥模式扫描，未发现 API Key、GitHub token、Bearer 字面量或私钥；干净分支生成后仍需复扫。
...
| 优先级 | 事项 | 当前判断 |
| --- | --- | --- |
| P0 交付 | 远端主线集成尚未完成 | 必须等 PR 必需检查 `ci` 通过并复核后合并 |
