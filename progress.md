...
- 本轮任务：审查并把阶段 7～10 集成到远端 `main`；远端事实记录为 PR #9。
- 本地源分支：`feat/phase-10-ai-provider` @ `564ec26`；`origin/main=5a85014`。
- 公开集成分支：`integrate/phase-10-mainline-20260720`，首个提交 `7551bbb`；CI、合并状态和最终 SHA 以 PR #9 为准。
...
1. PR #9 的必需检查 `ci` 必须成功。
2. PR changed files 不得出现 `docs/reviews/`、截图、真实文件清单或密钥。
3. PR 不得有未解决的阻断审查意见。
4. 合并状态和最终 merge SHA 直接从 PR #9 与 `origin/main` 核对；本文不固化会随合并过程变化的临时状态。
