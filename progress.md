...
- 阶段 3 完成：Watcher 现在按项目收集事件路径，并将新增、修改、删除以及移动的旧/新路径传给既有 `changed_paths` 快路径；冷却窗口改为延迟而非丢弃已收集事件。新增 `test_watcher_processor.py`，覆盖中文相对路径与新增、修改、删除、移动、冷却窗口内连续变更。定向 14 项、后端全量 92 项和 PR #5 的 WebView2/后端/前端 CI 通过；PR #5 已合并为 `f866b428657865fac876df8f1219027effe7af67`。阶段 1 的安装包早于此修复，下次桌面发布须重新构建并验收。
- Watcher 安装包复验：从 `main` 的 `fc747e46a414cfc4829a307873242f00c1df068f` 重建 `Project Vault_2.0.0-beta.1_x64-setup.exe`，SHA256 为 `74C58F116B25487A518345343442D2C664A6045990E3D21E7C15BF09B9DC9BC3`。隔离的 `scripts/verify_local_installed_usage.ps1` 验收 33/33 步通过（临时 fixture、临时安装目录、Mock Provider）；临时报告未提交。构建仅有既有 Next 静态导出 rewrites 提示。该包已包含 Watcher 修复；用户明确决定本次不执行 clean Windows 验收，本机安装验收即为当前交付门禁。
