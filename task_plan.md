...
结果：阶段 1 的 `2.0.0-beta.1` 安装包本机验收 33/33 步通过；阶段 2 的 PR #4 已在 CI 通过后合并为 `ba14023`；阶段 3 的 Watcher 修复通过定向 14 项和后端全量 92 项测试，并在 PR #5 CI 通过后合并为 `f866b42`。当前安装包不包含阶段 3 的 Watcher 修复；下一次安装包发布时应复用阶段 1 的本机验收流程。
